...
<main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    ...
    <!-- Page Header -->
    <div class="mb-12">
        <p class="text-primary font-medium tracking-wide uppercase text-xs mb-3">Your Reservations</p>
        <h1 class="text-4xl md:text-5xl font-extrabold text-darkCharcoal tracking-tight leading-tight mb-3">
            My Bookings
        </h1>
        <p class="text-gray-600 max-w-2xl">View and manage all your reservations. Complete payment for pending bookings below.</p>
    </div>
    ...
    <div class="mb-8 flex items-center gap-4">
        <form method="GET" action="{{ route('customer.bookings.index') }}" class="flex items-center gap-3">
            <label for="status-filter" class="text-sm font-bold text-on-surface">Filter by Status:</label>
            <select id="status-filter" name="status" onchange="this.form.submit()" class="px-4 py-2 border border-outline-variant/30 rounded-lg bg-white text-on-surface font-medium text-sm focus:outline-none focus:ring-2 focus:ring-primary">
        ...
        @if ($currentStatus)
            <a href="{{ route('customer.bookings.index') }}" class="text-primary text-sm font-bold hover:underline">Clear Filter</a>
        @endif
    </div>
                        ...
                        <tr class="hover:bg-surface-container-lowest/50 transition-colors">
                            ...
                            <td class="px-6 py-6 text-right">
                                @if ($booking->status === 'pending')
                                    ...
                                    <a href="{{ route('payment.show', $booking->id) }}" class="inline-flex items-center gap-2 px-4 py-2 bg-primary text-white font-bold text-sm rounded-lg hover:shadow-lg hover:shadow-primary/30 transition-all group">
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-6 py-12 text-center">
                                ...
                                <a href="{{ route('bookings.packages') }}" class="inline-flex items-center gap-2 px-6 py-3 bg-primary text-white font-bold rounded-lg hover:shadow-lg hover:shadow-primary/30 transition-all">
